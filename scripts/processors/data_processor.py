    ...
    for _, row in df.iterrows():
        try:
            if isinstance(row, dict):
                ...
                tags = row.get('tags', [])
                problem_patterns = row.get('problem_patterns', [])
                leetcode_indicators = row.get('leetcode_indicators', [])
                ...
                if not difficulty:
                    difficulty = determine_difficulty(complexity_time)
                ...
                problem_patterns = row['problem_patterns'] if 'problem_patterns' in row else []
                leetcode_indicators = row['leetcode_indicators'] if 'leetcode_indicators' in row else []
            ...
            algo_type = "technique" if any(t in tags for t in ["technique", "method", "approach"]) else "data structure" if any(t in tags for t in ["data structure", "data_structure"]) else "algorithm"
            
            # Create a comprehensive document with a more structured format
            content = f"""Algorithm Type: {algo_type}
            Algorithm: {name}
            ...
            Problem Patterns: {', '.join(problem_patterns) if problem_patterns else 'None'}
            Example Problems: {', '.join(leetcode_indicators) if leetcode_indicators else 'None'}
            """
            ...
            documents.append({
                ...
                "metadata": {
                    ...
                    "problem_patterns": problem_patterns if isinstance(problem_patterns, list) else [problem_patterns],
                    "examples": leetcode_indicators,
                }
            })
        except Exception as e:
...
# def extract_problem_patterns(name, description, tags):
#     """Extract common problem patterns this algorithm solves."""
#     patterns = []
#     name_desc = (name + " " + description).lower()
    
#     pattern_indicators = {
#         "substring problems": ["substring", "subarray", "contiguous"],
#         "path finding": ["path", "route", "navigation"],
#         "counting problems": ["count", "frequency", "occurrences"],
#         "optimization problems": ["maximize", "minimize", "optimal"],
#         "search problems": ["search", "find", "locate"],
#         "graph traversal": ["traversal", "visit", "explore"]
#     }
    
#     for pattern, indicators in pattern_indicators.items():
#         if any(indicator in name_desc for indicator in indicators):
